...
def sell_process() :
    ...
    for i in range(len(user)) :
        ...
        if pos[0]['result']['side'] == "Sell" :
            ...
            time.sleep(2)
            # 시장가 매수
            client.Order.Order_new(side="Sell",symbol="XRPUSD",order_type="Market",qty=int(size),time_in_force="GoodTillCancel").result()
            time.sleep(2)
            # 시장가 - 1000 지정가 예약
            client.Order.Order_new(side="Buy",symbol="XRPUSD",order_type="Limit",qty=int(size/2),price=marketPrice-0.005,time_in_force="GoodTillCancel").result()
            time.sleep(1)
            # 시장가 - 2000 지정가 예약
            client.Order.Order_new(side="Buy",symbol="XRPUSD",order_type="Limit",qty=int(size/2),price=marketPrice-0.01,time_in_force="GoodTillCancel").result()
            time.sleep(1)
            # 손절가 지정
