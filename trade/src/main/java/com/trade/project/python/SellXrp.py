...
def sell_process() :
    ...
    if pos[0]['result']['side'] == "Sell" :
        ...
        # 매수 잔량 청산
        client.Order.Order_new(side="Sell",symbol="XRPUSD",order_type="Market",qty=int(buySize),time_in_force="GoodTillCancel").result()
        # 시장가 매수
        client.Order.Order_new(side="Sell",symbol="XRPUSD",order_type="Market",qty=4,time_in_force="GoodTillCancel").result()
        # 시장가 - 1000 지정가 예약
        client.Order.Order_new(side="Buy",symbol="XRPUSD",order_type="Limit",qty=2,price=marketPrice-1000,time_in_force="GoodTillCancel").result()
        # 시장가 - 2000 지정가 예약
        client.Order.Order_new(side="Buy",symbol="XRPUSD",order_type="Limit",qty=1,price=marketPrice-2000,time_in_force="GoodTillCancel").result()
        # 시장가 - 3000 지정가 예약
        client.Order.Order_new(side="Buy",symbol="XRPUSD",order_type="Limit",qty=1,price=marketPrice-3000,time_in_force="GoodTillCancel").result()
