...
def buy_process() :
    ...
    if pos[0]['result']['side'] == "Buy" :
        ...
        # 매도 잔량 청산
        client.Order.Order_new(side="Buy",symbol="ETHUSD",order_type="Market",qty=int(buySize),time_in_force="GoodTillCancel").result()
        # 시장가 매수
        client.Order.Order_new(side="Buy",symbol="ETHUSD",order_type="Market",qty=6520,time_in_force="GoodTillCancel").result()
        # 시장가 + 1000 지정가 예약
        client.Order.Order_new(side="Sell",symbol="ETHUSD",order_type="Limit",qty=3260,price=marketPrice+1000,time_in_force="GoodTillCancel").result()
        # 시장가 + 2000 지정가 예약
        client.Order.Order_new(side="Sell",symbol="ETHUSD",order_type="Limit",qty=1630,price=marketPrice+2000,time_in_force="GoodTillCancel").result()
        # 시장가 + 3000 지정가 예약
        client.Order.Order_new(side="Sell",symbol="ETHUSD",order_type="Limit",qty=1630,price=marketPrice+3000,time_in_force="GoodTillCancel").result()
