...
def cal_dmi() :
    ...
    for i in range(len(sinceArr)) :
        ...
        # Data 호출
        response = session.query_kline(symbol='BTCUSD', interval=240, **{'from':since})['result']
        # Pandas Data 파싱
    ...
    trChoose = [0]
    client = bybit.bybit(test=False, api_key='vdxzqZLxzJtJaemOgX', api_secret='Xs2atrYxi1llNO3mDHqTTMYKrO0nS5doPzBx')
    trData = client.Kline.Kline_get(symbol="BTCUSD", interval="240", **{'from':unixtime-1209600}).result()[0]['result']
